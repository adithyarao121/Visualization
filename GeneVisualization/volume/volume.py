...
class GradientVolume:
    ...
    def get_gradient(self, x, y, z):
        if (x + self.volume.dim_x * (
                y + self.volume.dim_y * z) >= self.volume.dim_x * self.volume.dim_y * self.volume.dim_z):
            return 0
    ...
    def set_alpha(self, x, y, z, value):
        self.alpha[x + self.volume.dim_x * (y + self.volume.dim_y * z)] = value

    def get_alpha(self, x, y, z):
        if (x + self.volume.dim_x * (
                y + self.volume.dim_y * z) >= self.volume.dim_x * self.volume.dim_y * self.volume.dim_z):
            return 0
        return self.alpha[x + self.volume.dim_x * (y + self.volume.dim_y * z)]
    ...
    def compute(self):
        ...
        self.set_gradient(self.volume.dim_x - 1, self.volume.dim_y - 1, self.volume.dim_z - 1, 0)
